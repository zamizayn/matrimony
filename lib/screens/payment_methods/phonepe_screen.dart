...
class _PhonepeScreenState extends State<PhonepeScreen> {
  ...
  void initState() {
    ...
    super.initState();
    initialUrl =
        "${AppConfig.BASE_URL}/pay-with-phonepe?amount=${widget.amount}&payment_method=${widget.payment_method_key}&payment_type=${widget.payment_type}&package_id=${widget.package_id.toString()}";
    phonePe();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar:
          CommonAppBar(text: AppLocalizations.of(context)!.phonepe_screen_title)
              .build(context),
      body: buildBody(),
    );
  }
